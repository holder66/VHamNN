...
```sh
v run . rank -w -s datasets/prostata.tab 
```   
```sh
...
```sh
v run . rank -w -s -b 6 datasets/prostata.tab
```
```sh
Attributes Sorted by Rank Value, including missing values
...
```sh
v run . explore -c -e -g -w -b 2,12 -a 1,20 datasets/prostata.tab
```
...
```sh
v run . explore -c -e -g -w -b 6,12 -a 3,5 -u datasets/prostata.tab
```
```sh
A correct classification to "normal" is a True Positive (TP);
A correct classification to "tumor" is a True Negative (TN).
...
```sh
v run . make -a 3 -b 6 -u -c -o ../classifiers/prostata_classifier datasets/prostata.tab
```
